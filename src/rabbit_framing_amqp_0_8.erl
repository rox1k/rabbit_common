...
-type(amqp_method_field_name() ::
       ( consumer_tag | operation | nowait | consumer_tag
       | queue | ticket | exchange | content_size
       | type | delivery_tag | passive | integer_result
       | durable | global | delivery_tag | auto_delete
       | immediate | internal | nowait | arguments
       | string_2 | operation | delivery_tag | ticket
       | exchange | consumer_tag | consumer_tag | if_unused
       | nowait | routing_key | message_count | identifier
       | table | prefetch_count | cluster_id | redelivered
       | integer_op | consumer_tag | ticket | nowait
       | queue | passive | durable | exclusive
       | exchange | content_checksum | auto_delete | routing_key
       | nowait | routing_key | arguments | integer_2
       | requeue | global | queue | message_count
       | delivery_tag | multiple | routing_key | consumer_count
       | reply_code | exchange | no_local | string_result
       | exclusive | ticket | consumer_tag | queue
       | exchange | channel_max | ticket | mandatory
       | routing_key | exchange | nowait | arguments
       | consumer_tag | nowait | queue | queue
       | exchange | ticket | dtx_identifier | redelivered
       | queue | nowait | mandatory | immediate
       | version_major | version_minor | exchange | routing_key
       | global | message_count | mechanisms | multiple
       | locales | ticket | reply_text | queue
       | if_unused | integer_3 | queue | if_empty
       | ticket | prefetch_size | locale | ticket
       | requeue | prefetch_count | reply_text | challenge
       | no_ack | message_count | arguments | consumer_tag
       | immediate | response | ticket | no_local
       | delivery_tag | ticket | frame_max | identifier
       | heartbeat | routing_key | exclusive | exchange
       | channel_max | server_properties | frame_max | heartbeat
       | nowait | virtual_host | exchange | capabilities
       | exclusive | reply_code | delivery_tag | insist
       | requeue | known_hosts | prefetch_size | prefetch_count
       | host | known_hosts | string_1 | delivery_tag
       | reply_code | reply_code | reply_text | consumer_tag
       | class_id | method_id | consumer_tag | redelivered
       | reply_text | no_local | no_ack | out_of_band
       | integer_4 | nowait | ticket | active
       | requeue | consumer_tag | client_properties | active
       | queue | exchange | staged_size | reply_code
       | reply_text | consume_rate | meta_data | details
       | routing_key | mechanism | identifier | ticket
       | reply_code | consumer_tag | reply_text | exchange
       | class_id | result | string_op | method_id
       | exchange | nowait | exchange | routing_key
       | response | mandatory | no_ack | routing_key
       | realm | queue | exclusive | result
       | passive | consumer_tag | active | prefetch_size
       | nowait | write | consumer_tag | read
       | routing_key | ticket | delivery_tag | consumer_tag
       | integer_1 | ticket )).
