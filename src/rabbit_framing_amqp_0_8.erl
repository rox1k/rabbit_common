...
-type(amqp_method_field_name() ::
       ( exclusive | server_properties | mechanisms | locales
       | ticket | consumer_tag | client_properties | mechanism
       | routing_key | exclusive | message_count | response
       | requeue | delivery_tag | consumer_tag | locale
       | prefetch_count | challenge | routing_key | message_count
       | cluster_id | response | delivery_tag | delivery_tag
       | nowait | string_op | channel_max | frame_max
       | heartbeat | routing_key | channel_max | redelivered
       | reply_code | frame_max | routing_key | heartbeat
       | integer_2 | nowait | consumer_tag | virtual_host
       | capabilities | delivery_tag | insist | reply_text
       | requeue | exchange | string_result | known_hosts
       | queue | prefetch_count | host | known_hosts
       | routing_key | queue | reply_code | ticket
       | reply_text | consumer_tag | mandatory | class_id
       | method_id | consumer_tag | no_local | if_unused
       | no_ack | identifier | out_of_band | nowait
       | exchange | immediate | active | consumer_tag
       | prefetch_size | if_empty | active | consumer_tag
       | multiple | exchange | reply_code | reply_text
       | details | integer_3 | identifier | nowait
       | nowait | reply_code | reply_text | class_id
       | reply_text | method_id | no_ack | exchange
       | consumer_tag | routing_key | identifier | global
       | no_local | delivery_tag | version_major | immediate
       | realm | exclusive | passive | exchange
       | active | delivery_tag | write | prefetch_size
       | read | routing_key | ticket | redelivered
       | integer_1 | content_checksum | consumer_tag | immediate
       | prefetch_count | ticket | consumer_tag | exchange
       | string_1 | type | passive | durable
       | auto_delete | exchange | routing_key | reply_text
       | internal | delivery_tag | redelivered | nowait
       | consumer_tag | arguments | dtx_identifier | operation
       | reply_code | ticket | queue | multiple
       | exchange | reply_code | if_unused | integer_4
       | nowait | ticket | message_count | table
       | integer_op | ticket | ticket | ticket
       | ticket | queue | queue | staged_size
       | passive | result | durable | queue
       | meta_data | exclusive | auto_delete | routing_key
       | queue | nowait | exchange | exchange
       | global | arguments | requeue | string_2
       | queue | no_ack | nowait | message_count
       | prefetch_size | consumer_count | exchange | routing_key
       | no_local | delivery_tag | exclusive | ticket
       | mandatory | exchange | result | queue
       | consumer_tag | exchange | routing_key | nowait
       | mandatory | arguments | global | consumer_tag
       | consumer_tag | nowait | ticket | consume_rate
       | consumer_tag | operation | requeue | arguments
       | queue | version_minor | ticket | ticket
       | queue | nowait | ticket | integer_result
       | exchange | content_size )).
